function result = aggregateGrowingResults(resultFolderCorrected, absResultFolder, binsToCompute, exportPlots, p)
    debug = true;
    ...
    longestFile = load([resultFolderCorrected 'vm' filesep 'simVm.mat']);
    ...
    nReplicates = length(longestFile.result.simResult);
    maxLengthOverall = longestFile.result.vm.binnedProfileMaxLengthOverall;
    maxIndOverall = 2*longestFile.result.vm.binnedProfileMaxLengthOverall + longestFile.result.vm.binnedPeakOverhangOverall - 1; %% FOO?
%     for j = 1:nReplicates
...
%     end
    ...
    nTimeSteps = size(longestFile.result.vm.mtState, 1);
    ...
    for bin = binsToCompute'
        ...
        %currentFile = load([resultFolderCorrected.absVmFolderName{j} int2str(i) '.mat']);   
        result.vmBin{binIndex}.meanBoundKip2s = mean(sum(longestFile.result.vm.mtStateNoNan, 2));
        for fileJ = 1:binSize
            for fileI = 1:nTimeSteps
                peakIndex = longestFile.result.vm.binnedPeakLocations(fileI, fileJ);
                if isnan(peakIndex)
                    continue;
                end
                peakLocation = longestFile.result.vm.xCoordsBinned((peakIndex - 1)*binSize + fileJ);
                ...
                result.vmBin{binIndex}.allAlignedBinnedProfiles(profileIndex, (startIndMinus1 + 1):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 1)) = longestFile.result.vm.binnedProfiles{fileJ}(fileI, :);
...
%                     end
                result.vmBin{binIndex}.allAlignedBinnedProfiles(profileIndex, (startIndMinus1 + 2):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 2)) = 0.5 * (...
                    result.vmBin{binIndex}.allAlignedBinnedProfiles(profileIndex, (startIndMinus1 + 1):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 3)) + ...
                    result.vmBin{binIndex}.allAlignedBinnedProfiles(profileIndex, (startIndMinus1 + 3):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 1)) ...
                );                    
                result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, (startIndMinus1 + 1):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 1)) = longestFile.result.vm.binnedXCoords{fileJ};

                result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, (startIndMinus1 + 2):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 2)) = 0.5 * (...
                    result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, (startIndMinus1 + 1):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 3)) + ...
                    result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, (startIndMinus1 + 3):2:(startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :)) - 1)) ...
                );               
                for k = startIndMinus1:-1:1
                    result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, k) = result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, k+1) - longestFile.result.vm.params.binSize*0.008 * 0.5;
                end

                for k = (startIndMinus1 + 2*length(longestFile.result.vm.binnedProfiles{fileJ}(fileI, :))):maxIndAll
                    result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex,k) = result.vmBin{binIndex}.allAlignedBinnedProfileCoords(profileIndex, k-1) + longestFile.result.vm.params.binSize*0.008 * 0.5;
                end           
                ...
                result.vmBin{binIndex}.mtLengths(profileIndex) = longestFile.result.simResult{1}.parameters.endLength;
